...
namespace EmployeeTask.API.Controllers
{
    ...
    public class LaborCostController : ControllerBase
    {
        ...
        public LaborCostController(IDbRepository<LaborCost> laborCostsRepository)
        {
        ...
        /// </summary>
        /// <param name="id">уникальный ключ</param>
        /// <returns>Статус http 200 если данные о трудозатратах найден и удалены, 404 если данных о трудозатратах с нужным id нет, 500 - ошибка сервера при удалении данных о трудозатратах</returns>
        [HttpDelete("{id:int}")]
        [ProducesResponseType(typeof(LaborCost), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(string), StatusCodes.Status404NotFound)]
        [ProducesResponseType(typeof(string), StatusCodes.Status500InternalServerError)]
